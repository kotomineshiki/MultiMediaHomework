...
Y_en=DPCM_RLE_Decoding(Y_RLE,Y_DPCM,size(Y_dct_q));
Cb_en=DPCM_RLE_Decoding(Cb_RLE,Cb_DPCM,size(Cb_dct_q));
Cr_en=DPCM_RLE_Decoding(Cr_RLE,Cr_DPCM,size(Cr_dct_q));


 
%对三个通道分别反量化和反DCT
Y_in_q_dct=Inverse_Quantize_Dct(Y_en,Qua_Factor,Y_Table);
Cb_in_q_dct=Inverse_Quantize_Dct(Cb_en,Qua_Factor,CbCr_Table);
Cr_in_q_dct=Inverse_Quantize_Dct(Cr_en,Qua_Factor,CbCr_Table);
%  
%恢复出YCBCR图像
